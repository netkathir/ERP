...
@section('title', 'Add Raw Material - ERP System')
    ...
    <form action="{{ route('raw-materials.store') }}" method="POST" id="rawMaterialForm">
        ...
        <div style="margin-bottom: 20px;">
            ...
            <select name="raw_material_sub_category_id" id="raw_material_sub_category_id"
                style="width: 100%; padding: 12px; border: 1px solid #ddd; border-radius: 5px; font-size: 14px; background: white;">
                <option value="">Select Raw Material Category first</option>
            </select>
        ...
        <div style="margin-bottom: 20px;">
            ...
            <select name="batch_required" id="batch_required" required
                style="width: 100%; padding: 12px; border: 1px solid #ddd; border-radius: 5px; font-size: 14px; background: white;">
                <option value="No" {{ old('batch_required', 'No') == 'No' ? 'selected' : '' }}>No</option>
                <option value="Yes" {{ old('batch_required') == 'Yes' ? 'selected' : '' }}>Yes</option>
            </select>
        ...
        <div style="margin-bottom: 20px;">
            ...
            <select name="qc_applicable" id="qc_applicable" required
                style="width: 100%; padding: 12px; border: 1px solid #ddd; border-radius: 5px; font-size: 14px; background: white;">
                <option value="No" {{ old('qc_applicable', 'No') == 'No' ? 'selected' : '' }}>No</option>
                <option value="Yes" {{ old('qc_applicable') == 'Yes' ? 'selected' : '' }}>Yes</option>
            </select>
            @error('qc_applicable')
                <p style="color: #dc3545; font-size: 12px; margin-top: 5px;">{{ $message }}</p>
            @enderror
        ...
        <div style="margin-bottom: 20px;">
            ...
            <select name="test_certificate_applicable" id="test_certificate_applicable" required
                style="width: 100%; padding: 12px; border: 1px solid #ddd; border-radius: 5px; font-size: 14px; background: white;">
                <option value="No" {{ old('test_certificate_applicable', 'No') == 'No' ? 'selected' : '' }}>No</option>
                <option value="Yes" {{ old('test_certificate_applicable') == 'Yes' ? 'selected' : '' }}>Yes</option>
            </select>
        ...
        <div style="margin-bottom: 20px;">
            ...
            <textarea name="sop" id="sop" rows="4"
                ...
                placeholder="Enter SOP for handling the raw material">{{ old('sop') }}</textarea>
            @error('sop')
                <p style="color: #dc3545; font-size: 12px; margin-top: 5px;">{{ $message }}</p>
            @enderror
        </div>
...
<script>
    ...
    function clearForm() {
        ...
        document.getElementById('raw_material_sub_category_id').innerHTML = '<option value="">Select Raw Material Category first</option>';
        document.getElementById('raw_material_sub_category_id').disabled = false;
